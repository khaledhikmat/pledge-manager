...
try
{
    ...
    if (command == "create")
    {
        ...
        {
            var url = $"{envService.GetBaseUrl("CAMPAIGNS")}/entities/institutions"; 
            Console.WriteLine($"Creating institutions URL: {url}");
            await Utilities.PostNoResponse<Institution>(httpClient, url, institution);
            //await daprClient.InvokeMethodAsync<Institution>(envService.GetCampaignsAppName(), $"entities/institutions", institution);    
        }

        Console.WriteLine("Creating users....");
        foreach (User user in users)
        {
            var url = $"{envService.GetBaseUrl("USERS")}/users"; 
            Console.WriteLine($"Creating users URL: {url}");
            await Utilities.PostNoResponse<User>(httpClient, url, user);
            //await daprClient.InvokeMethodAsync<User>(envService.GetUsersAppName(), $"users", user);    
        }

        Console.WriteLine("Creating campaigns....");
        foreach (Campaign campaign in campaigns)
        {
            var url = $"{envService.GetBaseUrl("CAMPAIGNS")}/entities/campaigns"; 
            Console.WriteLine($"Creating campaigns URL: {url}");
            await Utilities.PostNoResponse<Campaign>(httpClient, url, campaign);
            //await daprClient.InvokeMethodAsync<Campaign>(envService.GetCampaignsAppName(), $"entities/campaigns", campaign);    
        }
    }
    else if (command == "simulatedonors")
    {
        ...
        foreach(Donor donor in donors)
        {
            try
            {
                ...
                var url = $"{envService.GetBaseUrl("USERS")}/users/verifications/{donor.UserName}"; 
                ...
                url = $"{envService.GetBaseUrl("USERS")}/users/verifications/{donor.UserName}/{code}"; 
    ...
    else if (command == "simulatepledges")
    {
        ...
        while (pledges > 0)
        {
            ...
            try 
            {
                ...
                var url = $"{envService.GetBaseUrl("CAMPAIGNS")}/entities/campaigns/{campaign.Identifier}/pledges"; 
