...
public class InstitutionActor : Actor, IFundSinkActor
{
    ...
    public async Task ExternalizationTimerCallback(byte[] state)
    {
        ...
        await this._daprClient.SaveStateAsync<Institution>(Constants.DAPR_STORE_NAME, institution.Identifier, institution);
    ...
    private async Task<Institution> GetInstitutionState() 
    {
        ...
        if (!actorState.HasValue) 
        {
            ...
            var stateEntry = await _daprClient.GetStateEntryAsync<Institution>(Constants.DAPR_STORE_NAME, this.Id.ToString());
