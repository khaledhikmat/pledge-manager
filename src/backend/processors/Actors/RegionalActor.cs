...
public class RegionalActor : Actor, IFundSinkActor
{
    ...
    public async Task ExternalizationTimerCallback(byte[] state)
    {
        ...
        await this._daprClient.SaveStateAsync<FundSink>(Constants.DAPR_STORE_NAME, region.Identifier, region);
    }
    ...
    private async Task<FundSink> GetRegionState() 
    {
        ...
        if (!actorState.HasValue) 
        {
            ...
            var stateEntry = await _daprClient.GetStateEntryAsync<FundSink>(Constants.DAPR_STORE_NAME, this.Id.ToString());
