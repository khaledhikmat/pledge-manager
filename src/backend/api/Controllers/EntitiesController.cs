...
namespace pledgemanager.backend.api.Controllers;
...
public class EntitiesController : ControllerBase
{
    ...
    public async Task<ActionResult> GetCampaignById(string id, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            var stateEntry = await daprClient.GetStateEntryAsync<Campaign>(Constants.DAPR_STORE_NAME, id);
        ...
        try
        {
            ...
            if (0 == 1)
            {
                ...
                using var httpResponseMessage = await httpClient.PostAsync(
                    $"/v1.0-alpha1/state/{Constants.DAPR_STORE_NAME}/query?metadata.contentType=application/json", queryJson);
    ...
    public async Task<ActionResult> CreateCampaignAsync([FromBody]Campaign campaign, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            await daprClient.SaveStateAsync<Campaign>(Constants.DAPR_STORE_NAME, campaign.Identifier, campaign);
            return Ok();
        }
        catch (Exception e)
        {
    ...
    public async Task<ActionResult> UpdateCampaignAsync(string id, [FromBody]Campaign campaign, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            await daprClient.PublishEventAsync(Constants.DAPR_PUBSUB_NAME, Constants.DAPR_CAMPAIGNS_PUBSUB_TOPIC_NAME, campaign);
            return Ok();
        }
        catch (Exception e)
        {
    ...
    public async Task<ActionResult> CommandCampaignAsync(string id, [FromBody] CampaignCommand command, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            await daprClient.PublishEventAsync(Constants.DAPR_PUBSUB_NAME, Constants.DAPR_COMMANDS_PUBSUB_TOPIC_NAME, command);
            return Ok();
        }
        catch (Exception e)
        {
    ...
    public async Task<ActionResult> SubmitPledgeAsync(string id, [FromBody]Pledge pledge, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            {
                throw new Exception($"Campain [{id}] is not active!!!");
            }

            pledge.CampaignIdentifier = campaign.Identifier;
            _logger.LogInformation($"SubmitPledgeAsync - {pledge.Identifier}");
            await daprClient.PublishEventAsync(Constants.DAPR_PUBSUB_NAME, Constants.DAPR_PLEDGES_PUBSUB_TOPIC_NAME, pledge);
            return Ok();
        }
        catch (Exception e)
        {
    ...
    public async Task<ActionResult> GetInstitutionById(string id, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            var stateEntry = await daprClient.GetStateEntryAsync<Institution>(Constants.DAPR_STORE_NAME, id);
            return Ok(stateEntry != null ? stateEntry.Value : null);
        }
        catch (Exception e)
        {
    ...
    public async Task<ActionResult> CreateInstitutionAsync([FromBody]Institution institution, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            await daprClient.SaveStateAsync<Institution>(Constants.DAPR_STORE_NAME, institution.Identifier, institution);
            return Ok();
        }
        catch (Exception e)
        {
    ...
    public async Task<ActionResult> GetFundSinkById(string id, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            var stateEntry = await daprClient.GetStateEntryAsync<FundSink>(Constants.DAPR_STORE_NAME, id);
            return Ok(stateEntry != null ? stateEntry.Value : null);
        }
        catch (Exception e)
        {
    ...
    public async Task<ActionResult> CreateFundSinkAsync([FromBody]FundSink fundsink, [FromServices] DaprClient daprClient)
    {
        try
        {
            ...
            await daprClient.SaveStateAsync<FundSink>(Constants.DAPR_STORE_NAME, fundsink.Identifier, fundsink);
            return Ok();
        }
        catch (Exception e)
        {
